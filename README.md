...
- **Per-link publishing via `bulkUpdate` status injection.** Each affiliate-link card has a per-link `draft`/`published` state separate from the shop-level Published toggle. The visible UI controls (per-link `role="switch"` and the form's `Update` button) cannot reliably set this — clicking the switch fires zero network requests, and `updatePartnerShopFromForm`'s `partner_shop_links[].status` field is silently discarded by the backend when `bulkUpdatePartnerShopLinks` has touched the same links recently (verified via mutation trace + reload comparison: mutation args show `published=N`, backend stores `draft=N`). The working path: the `bulkUpdatePartnerShopLinks` mutation's `data` arg accepts a `status` field. Writeback installs a tiny `WebSocket.send` monkey-patch via `page.add_init_script` that intercepts `bulkUpdatePartnerShopLinks` frames and adds `status: 'published'` to `data`, so every selected affiliate link is published as part of the same operation that saves the cookie config. Verified end-to-end on Bulova (25/25), Just Lawnmowers (14/14).
