...
- **Coinfiliate quirk — per-link "draft" badge is purely cosmetic.** Each affiliate-link card shows a `draft` badge and contains a `role="switch"` toggle in its expanded form, but the switch fires zero network requests on click and its state never persists across a reload. Even fully-Published shops (VersedSkin, Alibaba LATAM, Just Lawnmowers, Bulova — verified) display `draft` on every per-link card. The only meaningful publish state is the shop-level toggle, which the automation handles correctly. Don't try to "fix" the per-link badges — they're a non-functional UI artifact on Coinfiliate's side.
